...
//		this.documents = loaded_documents;
	...
	private void loadAnnotations(File file) throws Exception {
		// read each line
		//get content, start and end
		List<String> lines = FileUtil.loadLineByLine(file);
	    for (String line : lines) 
	    	parseAnnotationLine(line,file);
	}
	private void parseAnnotationLine(String line, File file) throws Exception {
		
		//getting the line and token indexes
		String text_file_path = getTextFilePath(file.getAbsolutePath());
		...
		Artifact startArtifact = Artifact.findInstanceByStartIndex
			( text_file_partial_path, annotation.startChar, Type.Word);
...
//		}
		...
		if (file.getPath().endsWith("78.xml") && startArtifact.getStartIndex()== 2109)
		{
		...
		{
			if (startArtifact.getStartIndex()== 2109 && file.getPath().endsWith("78.xml"))
				break;
			curr_artifact =curr_artifact.getNextArtifact();
