...
public class I2b2TLinkAnnotationAnalyzer{
...
//				FileUtil.getFilesInDirectory(filesRoot,"tlink");
			...
			for(File file: files) {
				try{
					generatePhraseLink(file.getAbsolutePath());
				}catch (IOException e){
					e.printStackTrace();
				}
				counter++;
	...
	private void generatePhraseLink(String filePath) throws Exception {
...
//		if (text_file_path.matches(".*426.*") || text_file_path.matches(".*386.*") || text_file_path.matches(".*422.*"))
//			return;
...
//				toPhrase =Phrase.getInstance("Discharge", "Discharge","MADEUP");
			...
			phrase_link.setLinkType(LinkType.valueOf(tagged_link.linkType));
			phrase_link.setAltLinkID(tagged_link.linkAltId);
